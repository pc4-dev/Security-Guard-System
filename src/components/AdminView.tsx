  ...
  const handleExportExcel = () => {
    try {
      const exportData = filteredLogs.map((log, index) => ({
        ...
        'Checkpoint-1 Image': log.image1Url || (log.photoUrls && log.photoUrls.length > 0 ? log.photoUrls[0] : ''),
        'Checkpoint-2 Image': log.image2Url || (log.photoUrls && log.photoUrls.length > 1 ? log.photoUrls[1] : ''),
        'Checkpoint-3 Image': log.image3Url || (log.photoUrls && log.photoUrls.length > 2 ? log.photoUrls[2] : ''),
        'Checkpoint-4 Image': log.image4Url || (log.photoUrls && log.photoUrls.length > 3 ? log.photoUrls[3] : ''),
        'Checkpoint-5 Image': log.image5Url || (log.photoUrls && log.photoUrls.length > 4 ? log.photoUrls[4] : ''),
        'Checkpoint-6 Image': log.image6Url || (log.photoUrls && log.photoUrls.length > 5 ? log.photoUrls[5] : '')
      }));
